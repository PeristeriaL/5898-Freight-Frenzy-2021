 ...
 */
@Autonomous(name = "Cam Blue Left- Use speed", group = "Blue")

public class AutoBlueLeftS extends LinearOpMode {
  ...
  public void top() {
    forwardAmt(90, .6);//get away from the wall a tad
    strafe(1100, .4);//strafe to line up with the goal
    rotate(440, .3);//rotate from start position
    up(2700, .6);//bring bucket up

    forwardAmt(770, .55);//go forward to goal
    //rotate(100, .3);//rotate to place box

    outtake();

    rotate(-100,.1);//rotate back
    backAmt(100, .3);//back up a little bit    
    up(-2700, .6);//go down
    rotate(-200, .3);//rotate back to safe position for going full send over the barrier
