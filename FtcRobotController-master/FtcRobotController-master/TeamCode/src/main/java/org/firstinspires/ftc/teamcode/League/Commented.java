 ...
 */
@Autonomous(name = "Cam Blue Left SPEEEEEED (top only)", group = "Blue") //naming your files is
    //important. dont be william beard with "auto 4" and "auto 10" because thats bad. the groups
    // are also nice because it makes a little line and separates them. this year i used blue and red

public class Commented extends LinearOpMode {  //you MUST have the "Commented" match your file name.
                                                // if its a new file and doesnt compile, check this

    /* declaring variables-
        private- all of your things you declare will be private. this is strict in APCSA and you
            should do it here too. private just means you cant access it outside of the class
        after private is the object type you are delcaring. this can be String, DcMotor, int, etc.
        next you can just list out all of the items of that type seperated by a comma since java
            good and lets you do it in one line
     */
    ...
    //ok so here is where you change what tflite file you use. you can use the FIRST Machine Learning software online
    //to create your own as yall know
    private static final String TFOD_MODEL_ASSET = "UltimateGoal.tflite"; // ty ftc for not changing file names year to year <3

    //below is an array. java has arrays and arraylists, if you want to learn more about these just know
    //there are 2 kinds. we dont use them much here but they do appear occasionally like below
    private static final String[] LABELS = { "Box", "Duck", "Ball", "Marker" };

    //this is the vuforia key from james' code from last year since mine didnt work
    ...
    public void runOpMode() {
        ...
        initVuforia();  //these methods just initialize tensorflow and vuforia
        initTfod();
        if (tfod != null) {
            tfod.activate();
//their github wrote whats below, i found it helpful when changing the tfod.setZoom.
...
//when you use telemetry, you use .addData(String, String) and make sure you update() it after so
// it will appear on screen. personally, i dont understand it enough so i sparcely used it but
// it can be very useful  for debugging.
        ...
        //ok so in the tutorial for this you literally can copy and paste this in because theres no way
        //to expect some high schoolers to know how to do this kinda stuff. well, at least i had no idea
        //its commented anyways because why not
        if (opModeIsActive()) {
            ...
            while (opModeIsActive()) {
                counter++; //this is why it takes a second for thanos to initialize, this runs 5000 times. good job thanos <3
                if (tfod != null) {
                    // getUpdatedRecognitions() will return null if no new information is available
                    // since
                    // the last time that call was made.
                    List<Recognition> updatedRecognitions = tfod.getUpdatedRecognitions();
                    //above is an arrayList. again, if youre interested in this you can google it now
                    // but it seldom appears here and when it does it is not used much.
                    ...
                    if (updatedRecognitions != null) {
                        ...
                        for (Recognition recognition : updatedRecognitions) {
                            ...
                            else if (recognition.getLeft() > recognition.getTop()) { //right side
                                level = "top";
                            }
                            else {
                                level = "bottom";
                ...
                if (counter >= 5000)
                    break; //this will end the while loop and break out of it
            }
            // break to here

            telemetry.addData("alrighty, ima put this block on the ", level + "level.");
            telemetry.update();

            // wobble goal tape 1 code-> [duck tape tape]
            if (level.equals("bottom")) lowest();
                //java is great because if you are doing 1 line if statements you do not need {}
                //this lets you put it on the same line or just not put brackets

            // wobble goal tape 2 code-> [tape duck tape]
            else if (level.equals("middle")) { mid();}
                //you can also do this

            // wobble goal tape 3 code-> [tape tape duck]
            else {
                top();
            } //or this if youre a loser
