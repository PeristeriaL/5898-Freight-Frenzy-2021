 ...
 */
@Autonomous(name="Cam Red Left DUCK", group="Red")
@Disabled
public class AutoRedLeftDuck extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        bucketStart = screw.getCurrentPosition();
...
////// movement methods ////////////////////////////////////////////////////////
    ...
    public void forwardIntake(int amt, double power){        
        rf.setTargetPosition(rf.getCurrentPosition() + amt);
        rb.setTargetPosition(rb.getCurrentPosition() + amt);
        lf.setTargetPosition(lf.getCurrentPosition() + amt);
        lb.setTargetPosition(lb.getCurrentPosition() + amt);
    ...
    public void bucketToStart(){
        screw.setTargetPosition(bucketStart);
        screw.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        while(opModeIsActive()&&screw.isBusy()){
            screw.setPower(.7);
        }
...
// find out the randomizations so that the duck code goes to the right level
    ...
    public void lowest(){
        forwardAmt(90,.7); //get off wall
        strafe(1100,.5);//go to front of goal
        sleep(10);
        forwardAmt(690,.6);//approach goal
        sleep(50);
        outtake();//outtake
        
        forwardAmt(-400,.5);//back off goal
        strafe(-1450,.5);//strafe to duck
        up(-420,.7); //start to duck level
        rotate(560,.3);//rotate forward
        
        forwardIntake(500,.1);//ducknap
        
        rotate(-500,.3);//rotate back
        forwardAmt(-200,.4);//back up
        sleep(100);
        strafe(1450,.7);//approach goal
        ...
        sleep(500);
        up(-700,.6);//bring the bucket back down
        warehouse();
    }
    
    public void mid(){
        forwardAmt(90,.6); //get away from the wall
        strafe(1000,.4); // strafe to the goal
        up(1500,.9); //go up        
        ...
        up(-1500,.6);
        forwardAmt(-400,.5);//back off goal
        strafe(-1200,.5);//strafe to duck
        up(-420,.7); //start to duck level
        rotate(300,.3);//rotate forward
        ...
        bucketToStart();
        sleep(100);
        strafe(1170,.7);//approach goal
        
        up(1350,.9); //bring the bucket // -420 to 1500
        forwardAmt(200,.2);
        rotate(200,.3);
    ...
    public void top(){
        ...
        strafe(1000,.4);//strafe to be in front of the goal
        rotate(440,.3);//rotate from start position to be upright
        up(2700,.9); //bring the bucket
        forwardAmt(750,.55); //go forward to the goal
        rotate(50,.3);//rotate for drop position
        outtake();        
